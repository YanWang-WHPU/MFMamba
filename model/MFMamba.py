...
from model.swiftformer import SwiftFormerEncoder
from model.pkiattention import InceptionBottleneck
...
## 1x1
class MyModelPart(nn.Module):  
    def __init__(self, in_channels):  
        super(MyModelPart, self).__init__()  
        out_channels = in_channels // 3 * 2
        self.conv1x1 = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=1).to(device) 
...
class SeparableConv(nn.Sequential):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, dilation=1):
        super(SeparableConv, self).__init__(
            nn.Conv2d(in_channels, in_channels, kernel_size, stride=stride, dilation=dilation,
                      padding=((stride - 1) + dilation * (kernel_size - 1)) // 2,
                      groups=in_channels, bias=False), # 深度卷积层
            nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=False) # 逐点卷积层
        )
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.ReLU6, drop=0.):
        ...
        self.act = act_layer() # act_layer 
        self.fc2 = nn.Conv2d(hidden_features, out_features, 1, 1, 0, bias=True)
        self.drop = nn.Dropout(drop, inplace=True) # inplace=True 
...
class ChannelAtt(nn.Module):
    ...
    def forward(self, x):
        ...
        Att = torch.sigmoid(channel_att_sum).unsqueeze(2).unsqueeze(3).expand_as(x) # 归一化
        return Att
...
class FusionAttention(nn.Module):
    def __init__(self,
        ...
        ## 局部注意力
        # self.attns_local = nn.ModuleList()
        # self.attns_local = LocalAttention(ssmdims, heads, window_size_local, attn_drop, proj_drop)
        # self.model_part = MyModelPart(C_y,reduction_ratio=2)      
        
        # swiftformer 
        self.swiftformer = SwiftFormerEncoder(dim, mlp_ratio=mlp_ratio, drop_path=attn_drop,
                                              act_layer=act_layer, use_layer_scale=use_layer_scale,
                                               layer_scale_init_value=layer_scale_init_value)


        ## pkiattention
        self.pkiattention = InceptionBottleneck(ssmdims, dim, kernel_sizes, dilations,
        ...
        self.local3 = ConvBN(ssmdims, dim, kernel_size=5)
        self.proj = SeparableConvBN(dim, dim, kernel_size=window_size)
    ...
    def forward(self, x, y):
        ...
        # local = self.attns_local(y)
        local = self.pkiattention(y)

        out_G = self.swiftformer(x)
        ...
        # B_y, C_y, H_y, W_y = local.shape
        # y_local = local.clone() 
        # model_part = MyModelPart(C_y).to(device)
        # local = model_part(local)
...
class MFMamba(nn.Module):
    def __init__(self,
        ...
        self.tongdao = ConvBN(1,3,stride=1)                                
    ...
    def forward(self, x, y):
        ...
        ssmy = self.stem(y) 
        vss_outs = self.vssm_encoder(ssmy) # 48*128*128, 96*64*64, 192*32*32, 384*16*16, 768*8*8
