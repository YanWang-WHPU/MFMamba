...
FOLDER = "/home/dell/yanWang/data/" # Replace with your "/path/to/the/ISPRS/dataset/folder/"
BATCH_SIZE = 8 # Number of samples in a mini-batch

LABELS = ["Impervious surfaces", "buildings", "low vegetation", "trees", "cars", "clutter"] # Label names
...
def convert_to_color_troch(arr_2d, palette=palette):
    """ Numeric labels to RGB-color encoding """ 
    # assert arr_2d.dtype.is_integer, "Input tensor must have integer dtype" 
    # arr_3d = np.zeros((arr_2d.shape[0], arr_2d.shape[1], 3), dtype=np.uint8)
    print(arr_2d)
    arr_3d = torch.zeros((arr_2d.shape[0], arr_2d.shape[1], 3), dtype=torch.uint8)  
    color_tensors = {k: torch.tensor(v, dtype=torch.uint8) for k, v in palette.items()}  
  

    for label, color in color_tensors.items():  

        mask = arr_2d == label  
  

        arr_3d[mask, 0] = color[0]  
        arr_3d[mask, 1] = color[1]  
        arr_3d[mask, 2] = color[2]  
    # for c, i in palette.items():
        # m = arr_2d == c
        # arr_3d[m] = i
...
def metrics(predictions, gts, label_values=LABELS):
    ...
    ## Compute global accuracy，计算全局准确率 
    total = sum(sum(cm))
    ...
    for l_id, score in enumerate(F1Score):
        print("%s: %.4f" % (label_values[l_id], score)) # 打印每个类别的F1分数 
    print('mean F1Score: %.4f' % (np.nanmean(F1Score[:5]))) # 打印前5个类别的平均F1分数
    print("---")
    ...
    # 计算MIoU系数 
    MIoU = np.diag(cm) / (np.sum(cm, axis=1) + np.sum(cm, axis=0) - np.diag(cm)) # 计算每个类别的MIoU 
    print("classes IOU :")
    print(MIoU) # 打印每个类别的MIoU 
    MIoU = np.nanmean(MIoU[:5]) # 计算前5个类别的平均MIoU 
    print('mean MIoU: %.4f' % (MIoU)) # 打印平均MIoU
    print("---")
